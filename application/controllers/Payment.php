<?php
require_once("Home.php"); // Inclusion du contrôleur home
...
class Payment extends Home
{
    public function __construct()
    {
        ...
        // Charger le fichier de configuration Moncash
    require_once(APPPATH . 'config/moncash.php');
    ...
    public function getExchangeRate() {
        $url = 'https://api.brh.gouv.ht/exchange_rate'; // Remplacez par l'URL correcte
        ...
        // Assurez-vous de vérifier la structure des données
        return $data['rate']; // Ajustez cela selon la réponse de l'API
    }
    

    public function init_payment() {
        // Vérifie que CLIENT_ID et CLIENT_SECRET sont bien définis
        if (!defined('CLIENT_ID') || !defined('CLIENT_SECRET')) {
            throw new Exception("Les constantes CLIENT_ID et CLIENT_SECRET ne sont pas définies.");
        }
        
        // Créez une instance de Moncash
        $moncash = new \Mds\Moncash\Moncash(CLIENT_ID, CLIENT_SECRET);
        
        // Récupérez le montant envoyé par le formulaire
        $amount = $this->input->post('amount'); // Assurez-vous que ce champ est présent dans le formulaire
    
        // Vérifiez que le montant est valide
        if (is_numeric($amount) && $amount > 0) {
            // Créez une commande unique
            $order_id = 'ORDER-' . uniqid(); // Exemple d'ID de commande
    
            // Appelez la méthode makePayment avec le montant dynamique
            $payment = $moncash->makePayment($order_id, $amount);
            $url = $payment->getRedirect();
    
            // Redirigez vers l'URL de paiement
            redirect($url);
        } else {
            // Gérez l'erreur : montant invalide
    ...
    public function accounts_action()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('moncash_enabled', '<b>' . $this->lang->line("Moncash checkout") . '</b>', 'required|in_list[0,1]');

            //validation Moncashpay
            $this->form_validation->set_rules('moncashpay_client_id', '<b>' . $this->lang->line("Moncashpay Client ID") . '</b>', 'trim|required');
            $this->form_validation->set_rules('moncashpay_secret_id', '<b>' . $this->lang->line("Moncashpay Secret ID") . '</b>', 'trim|required');
            $this->form_validation->set_rules('moncash_mode', '<b>' . $this->lang->line("Moncashpay Mode") . '</b>', 'trim');
            ...
            if ($this->form_validation->run() == false) {
                ...
                //moncashpay assign
                $moncashpay_client_id = strip_tags($this->input->post('moncashpay_client_id', true));
                $moncashpay_secret_id = strip_tags($this->input->post('moncashpay_secret_id', true));
                $moncash_mode = strip_tags($this->input->post('moncash_mode', true));
                $moncash_enabled = $this->input->post('moncash_enabled', true);
                ...
                $update_data =
                    array(
                        ...
                        'moncashpay_client_id' => $moncashpay_client_id,
                        'moncashpay_secret_id' => $moncashpay_secret_id,
                        'moncash_mode' => $moncash_mode,
                        'moncash_enabled' => $moncash_enabled,
