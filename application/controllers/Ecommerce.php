...
public function moncash_action($store_id='', $cart_id='', $subscriber_id='', $payment_amount='', $currency='', $store_name='')
{ 
  // Charger la bibliothèque MonCash
  $this->load->library('moncash_class_ecommerce');
  $ecommerce_config =  $this->get_ecommerce_config($store_id);
  $moncash_client_id = isset($ecommerce_config['moncash_client_id']) ? $ecommerce_config['moncash_client_id'] : '';
  $moncash_client_secret = isset($ecommerce_config['moncash_client_secret']) ? $ecommerce_config['moncash_client_secret'] : '';
  $this->moncash_class_ecommerce->client_id = $moncash_client_id;
  $this->moncash_class_ecommerce->client_secret = $moncash_client_secret;
  
  // Effectuer le paiement MonCash
  $response = $this->moncash_class_ecommerce->moncash_payment_action($payment_amount, $currency, $store_name);
  if ($response['status'] == 'Error') {
    echo $response['message'];
    exit();
  }
  ...
  if ($currency == 'HTG') $payment_amount = $response['charge_info']['amount'];
  else $payment_amount = $response['charge_info']['amount']; // Adapter selon la conversion nécessaire

  $transaction_id = $response['charge_info']['transaction_id'];
  $payment_date = date("Y-m-d H:i:s", $response['charge_info']['created']);
  $country = 'HT'; // MonCash est spécifique à Haïti

  $moncash_transaction_source = isset($response['charge_info']['source']) ? $response['charge_info']['source'] : "";
  $moncash_transaction_source = json_encode($moncash_transaction_source);   
  ...
  $insert_data = array(
    ...
    'checkout_timestamp' => $payment_date,           
    'transaction_id' => $transaction_id,
    "checkout_source_json" => $moncash_transaction_source,            
    'paid_at' => $curtime,
    'status' => 'approved',
    'status_changed_at' => $curtime,
    'action_type' => 'checkout',
